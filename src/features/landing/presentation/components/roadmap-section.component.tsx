...
const StatusIcon = ({ status }: { status: string }) => {
    switch (status) {
        ...
        case 'in_progress':
            return <CircleDot className="h-6 w-6 text-brand-blue animate-pulse" />;
        default:
        ...
        <section className="w-full bg-background-dark py-20 sm:py-28">
            <div className="container mx-auto max-w-screen-xl px-4">
                ...
                <div className="relative mx-auto mt-20 max-w-5xl">
                    <div className="absolute left-1/2 top-0 hidden h-full w-1 -translate-x-1/2 roadmap-gradient-line md:block"></div>
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`
                                        ...
                                        ${isOdd ? 'md:self-end md:pl-10' : 'md:self-start md:pr-10'}
                                    `}
                                    initial="offscreen"
                                ...
                                >
                                    <div className={`
                                        w-full rounded-2xl border border-border-color p-6 
                                        transition-all duration-300
                                        hover:-translate-y-2
                                        ${stage.status === 'completed' ? 'bg-background-light/50 opacity-70' : 'bg-background-light'}
                                        ${stage.status !== 'completed' ? 'hover:border-brand-blue/50 hover:shadow-2xl hover:shadow-brand-blue/20' : ''}
                                    `}>
                                        <div className="flex items-center justify-between">
                                            <p className={`
                                                font-bold
                                                ${stage.status === 'in_progress' ? 'text-brand-blue' : 'text-text-secondary'}
                                            `}>
                                                {stage.date}
                                            </p>
